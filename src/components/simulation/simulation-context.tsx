...
interface SimulationContextType {
  ...
  advancePeriod: () => void;
  submitDecision: (
    decision: Omit<Decision, "id" | "processed" | "processedAt">
  ) => void;
  refreshState: () => void;
}
...
export const SimulationProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  useEffect(() => {
    try {
      ...
      const company = initialState.companies.find(
        (c) => c.id === "company_demo"
      );
      setUserCompany(company || null);
      // Get company products
      if (company) {
        const products = initialState.products.filter(
  ...
  const advancePeriod = () => {
    ...
    try {
      ...
      if (userCompany) {
        const updatedCompany = newState.companies.find(
          (c) => c.id === userCompany.id
        );
        setUserCompany(updatedCompany || null);

        if (updatedCompany) {
          const updatedProducts = newState.products.filter(
            (p) => p.companyId === updatedCompany.id
          );
  ...
  const refreshState = () => {
    ...
    try {
      ...
      // Update user company and products
      if (userCompany) {
        const updatedCompany = updatedState.companies.find(
          (c) => c.id === userCompany.id
        );
        setUserCompany(updatedCompany || null);
